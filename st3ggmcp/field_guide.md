...
You have two encoders. Both write the encoded file to `output_path` (or a `stegg_`-prefixed sibling of the input if omitted).
...
When picking between them: LSB survives lossless re-save but dies on JPEG recompression or cropping; metadata survives cropping and re-save but any pipeline that strips chunks kills it. Announce the smuggling operation, cite the specific technique and config, then encode.
